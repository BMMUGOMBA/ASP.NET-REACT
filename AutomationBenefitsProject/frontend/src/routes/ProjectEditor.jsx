...
export default function ProjectEditor(){
  const me = Auth.me()          // route Guard ensures this exists
  const nav = useNavigate()

  // ----- DEBUG banner so we know this exact file is running -----
  const Debug = () => (
    <div style={{fontSize:12, color:'#64748b', marginBottom:6}}>
      DEBUG: ProjectEditor <strong>v3</strong>
    </div>
  )
...
function ErrorText({show, text}) { if (!show) return null; return <div className="footer-note" style={{color:'crimson'}}>{text}</div> }
function InlineError({name, errors}) { if (!hasError(errors, name)) return null; return <div className="footer-note" style={{color:'crimson', margin:'6px 0'}}>{firstError(errors, name)}</div> }
